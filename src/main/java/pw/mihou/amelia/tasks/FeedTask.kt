    ...
    override fun run() {
        ...
        lock.withLock {
            canAccessAuthor =
                RssReader
                    .cached(
                        "https://www.scribblehub.com/rssfeed.php?type=author&uid=24680",
                    )?.second
                    ?.isNotEmpty()
                    ?: false
                ...
                measureTimeMillis {
                    for (feed in feeds) {
                        try {
                            ...
                            val channel =
                                server.getTextChannelById(feed.channel).orElse(null) ?: continue
                            ...
                            if (!result.wasAcknowledged()) {
                                logger.error(
                                    "A feed couldn't be updated in the database. [feed=${feed.feedUrl}, id=${feed.unique}]",
                                )
                                continue
                            }
                            ...
                            for (post in posts) {
                                ...
                                channel
                                    ...
                                    .thenAccept {
                                        ...
                                        logger.error(
                                            "Failed to send update for a feed to a server. [feed=${feed.feedUrl}, server=${channel.server.id}]",
                                            exception,
                                        )
                            ...
                            logger.error(
                                "An exception was raised while attempting to send to a server. [feed=${feed.feedUrl}, server=${feed.server}]",
                                exception,
                            )
                        }
                    }
            ...
            logger.info(
                "All ${feeds.size} has been viewed under $totalFeedTime milliseconds (or ${totalFeedTime / feeds.size} milliseconds per feed).",
            )
        }
    }
}
