...
public class Settings implements VelenEvent {

    @Override
    public void onEvent(MessageCreateEvent event, org.javacord.api.entity.message.Message msg, User user, String[] args) {
            ...
            if (args.length > 0) {
                if (args[0].equalsIgnoreCase("prefix")) {
                    ...
                    if (!msg.getMentionedRoles().isEmpty()) {
                        ServerDB.getServer(server.getId()).setRole(msg.getMentionedRoles().get(0).getId())
                                .update().thenAccept(unused -> Message.msg("We have now updated the configuration to allow " + msg.getMentionedRoles().get(0).getMentionTag() + " to edit, add, and remove RSS feeds and also subscribe roles to RSS feeds at will." +
                    ...
                    if (args.length > 1) {
                        String message = event.getMessageContent().substring(args[0].length() + 1);
