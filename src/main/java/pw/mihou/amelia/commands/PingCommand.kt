...
object PingCommand : NexusHandler {
                ...
                updater
                    .addEmbed(
                        EmbedBuilder()
                            ...
                            .setTitle("Ping pong!")
                            .setDescription(
                                "Here are my current metrics and information for this session!",
                            ).addField(
                                "Latency",
                                ...
                                StringUtils.createEmbeddedFormat(
                                    ...
                                    "💿 Memory: `" + format(getUsedMemory()) + " MB / " +
                                        format(os.totalMemorySize / (1000 * 1000)) +
                                        " MB`",
                                ),
                            ).addField(
                                "ScribbleHub Status",
                                StringUtils.createEmbeddedFormat(
                                    "\uD83D\uDD8B️ Author Feeds: ${booleanToEmoji(
                                        FeedTask.canAccessAuthor(),
                                    )}",
     ...
     */
    private fun format(number: Long): String? = NumberFormat.getInstance().format(number)
}
