    ...
    fun run(event: NexusCommandEvent, subcommand: SlashCommandInteractionOption) {
        ...
        event.respondLaterAsEphemeral().thenAccept { updater ->
            try {
                ...
                if (
                    !(host.equals("scribblehub.com", ignoreCase = true) || host.equals("rssscribblehub.com", ignoreCase = true)) ||
                ...
                if (result.wasAcknowledged()) {
                    updater.setContent("✅ I will try my best to send updates for the reading list ($name) in ${channel.mentionTag}!").update()
                    return@thenAccept
                }
