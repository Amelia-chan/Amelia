...
object RegisterAuthorSubcommand {
        ...
        event
            ...
            .thenAccept { updater ->
                if (subcommand.name == "user") {
                    future { Amaririsu.search(name) { series.enabled = false } }
                        .thenAccept connector@{ results ->
                            if (results.users.isEmpty()) {
                                updater
                                    .setContent(
                                        "❌ Amelia cannot found any users that matches the query, how about trying something else?",
                                    ).update()
                                return@connector
                            }

                            buttons(NexusPaginatorBuilder(results.users.toList()))
                                .setEventHandler(
                                    object : NexusPaginatorEvents<UserResultOrAuthor> {
                                        override fun onInit(
                                            ...
                                            buttonEvent.buttonInteraction.message
                                                ...
                                                .thenAccept update@{ message ->
                                                    ...
                                                    if (latestPost.date == null) {
                                                        message.edit(
                                                            TemplateMessages.ERROR_DATE_NOT_FOUND,
                                                        )
                                                        return@update
                                                    }
                                                    ...
                                                    if (result.wasAcknowledged()) {
                                                        message.edit(
                                                            "✅ I will try my best to send updates for ${cursor.item.name}'s stories in ${channel.mentionTag}!",
                                                        )
                                                        return@update
                                                    }

                                                    message.edit(
                                                        TemplateMessages.ERROR_DATABASE_FAILED,
                                                    )
                                                }.exceptionally {
                                                    it.printStackTrace()
                                                    ...
                                                    buttonEvent.buttonInteraction.message
                                                        .edit(
                                                            TemplateMessages.ERROR_FAILED_TO_PERFORM_ACTION,
                                                        )
                                ...
                                .send(event.event.interaction, updater)
                        }.exceptionally {
                            it.printStackTrace()
    ...
    private fun <Type> buttons(
        paginator: NexusPaginatorBuilder<Type>,
    ): NexusPaginatorBuilder<Type> =
        paginator
    ...
    private fun user(cursor: NexusPaginatorCursor<UserResultOrAuthor>) =
        EmbedBuilder()
            ...
            .setDescription(
                ...
                "You are looking at ${cursor.displayablePosition} out of ${cursor.maximumPages} pages",
