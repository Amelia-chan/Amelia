...
public class UnsubscribeCommand extends Command {
    ...
    public UnsubscribeCommand() {
        super("unsubscribe", "Unsubscribes a role to the feed.", "unsubscribe [feed unique id] [roles]", true);
    }
                ...
                try {
                    ...
                    if (FeedDB.validate(i)) {
                        FeedDB.getServer(server.getId()).getFeedModel(i).ifPresentOrElse(feedModel -> {
                            event.getMessage().getMentionedRoles().forEach(role -> feedModel.unsubscribeRole(role.getId()));
                            feedModel.update(server.getId());
                            Message.msg("We have unsubscribed the following roles: " + event.getMessage().getMentionedRoles().stream().map(Role::getMentionTag)
